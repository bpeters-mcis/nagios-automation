...
# Check to see if this user is in the lab attendant group
	...
	# Get all the windows team members, so we know who to give full access to.
	$winteamusers = $AD->getGroupusers('CN=doit-windows-team,CN=users,DC=ad,DC=emich,DC=edu');
	$winteam = array();

	$i = 0;
    	while ($i < $winteamusers['count']) {
        	array_push($winteam, $winteamusers[$i]['samaccountname'][0]);
        	$i++;
    	}
	...
	if (!in_array($_SERVER['REMOTE_USER'], $winteam)) {
		header( 'Location: https://winmon.emich.edu/nagios/cgi-bin/status.cgi?hostgroup=all&style=detail&hoststatustypes=15') ;
	}
